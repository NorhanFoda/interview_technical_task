...
abstract class BaseRepository implements BaseContract
{
     ...
     * @return mixed
     */
    public function sumWithFilters($column, $filters = []): float
    {
        $query = $this->withFilters($this->model, $filters);
        return $query->aggregate('sum', [$column]) ?? 0;
    }
    ...
    {
        $page = $data['page'] ?? true;
        $simplePaginate = $data['simplePaginate'] ?? false;
        ...
        if ($page) {
            return $simplePaginate ? $query->simplePaginate($limit) : $query->paginate($limit);
        }
        if ($limit) {
     ...
     */
    public function paginate(array|Collection $items, int $perPage = 15, null|int $page, array $options = []): LengthAwarePaginator
    {
